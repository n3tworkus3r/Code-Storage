...
def assignment_task():
    ...
    result = G.ford_fulkerson_algorithm()
    ...
    i = 1
    for chain in chain_list:
        performers.append(str(chain[1])[5:6])
        print('На {0}-ю работу назначен {1}-й исполнитель'.format(i,performers[i-1]))
        i+=1

    sum = 0

    """print('Стоимость назначения равна:')
    for i in range(n):
        for j in range(n):
            sum += C[i][int(performers[j])-1]
            print(str(C[i][int(performers[j])-1])+'+',end='')"""
